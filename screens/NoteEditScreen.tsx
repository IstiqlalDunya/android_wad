...
const NoteEditScreen = ({ route, navigation }: any) => {
    ...
    fetch(`${Config.settings.serverPath}/api/notes/detail/${noteId}`)
      ...
      .then((data) => {
        if (data) {
          ...
          setNoteDate(new Date(data.note_date));
  ...
  const handleUpdate = () => {
    ...
    fetch(`${Config.settings.serverPath}/api/notes/${noteId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
      body: JSON.stringify({ title: title.trim(), content: content.trim(), note_date: formatDate(noteDate) }),
    })
      .then((r) => { if (!r.ok) throw new Error('Failed to update note'); return r.json(); })
      .then((data) => {
        if (socket) socket.emit('note_updated', { id: noteId, ...data });
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.actionRow}>
        ...
        <AppButton title="Update Note" onPress={handleUpdate} style={styles.halfBtn} />
      </View>
    </ScrollView>
  );
};
...
const styles = StyleSheet.create({
  ...
  dateBtnText: { fontSize: 14, color: Colors.primary, fontWeight: '600' },
  contentInput: {
    minHeight: 220,
    fontSize: 15,
    color: Colors.textPrimary,
  ...
  actionRow: { flexDirection: 'row', gap: 10, marginTop: 8, marginBottom: 40 },
  halfBtn: { flex: 1 },
});
